...
public class Prototype implements Iterable<Column>{
	...
	public void addColumn(Column c) {
		if(c == null)
			throw new DataBaseException("Prototype->addColumn","Column passada � nula");
		columns.add(c);
	...
	public TranslatorApi validateColumns()  {
		...
		if(size()==0){
			String error="N�o � valido uma tabela com nenhuma coluna!";
		...
		for(short x=0;x<size();x++){
			...
			if(namelen>240 || namelen<1){
				String error="Coluna "+x+" tem nome de tamanhos inv�lido!";
				ex=new DataBaseException("Prototype->ValidateColumns",error);
			...
			if(col.isPrimaryKey() && col.camBeNull()){
				String error="Coluna "+col.getName()+" n�o pode ser nula e ao mesmo tempo ser primary key!";
				ex=new DataBaseException("Prototype->ValidateColumns",error);
				ex.addValidation("NULL not in PRIMARY KEY");
				throw ex;
			}
			if(col.isPrimaryKey() && col.isDinamicSize()){
				String error="Coluna "+col.getName()+" n�o pode ser dinamica ao mesmo tempo que � primary key!";
				ex=new DataBaseException("Prototype->ValidateColumns",error);
